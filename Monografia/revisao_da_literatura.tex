...
\[f_A(x) = \frac{f_O(x)}{ \sum_{i=1}^{n}  f_O(i)  }  \]
 ...
 A dinâmica populacional é a responsável pela evolução, ao propagar caracteristicas desejáveis a gerações subsequentes no processo de cruzamento, enquanto novas são testadas no processo de mutação.
