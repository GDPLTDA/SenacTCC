...
\textbf{Selection Roulette Wheel}: Faz a soma de todos os valores da função de aptidão da população, depois calcula a porcentagem de cada indivíduo referente ao total e guarda em um vetor. 
Então é gerado um valor X aleatório entre 0 a 1, e multiplicado pelo valor total dos pesos. 
Para selecionar o indivíduo é feito um loop no vetor de pesos e seus valores somados até que seja menor ou igual ao valor X aleatório calculado antes, a posição atual no vetor deste valor, será o utilizada para selecionar o individuo no vector de indivíduos.
